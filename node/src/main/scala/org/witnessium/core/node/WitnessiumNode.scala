...
object WitnessiumNode extends TwitterServer with ServingHtml with EncodeException {
  ...
  implicit val attachmentStore: KeyValueStore[IO, UInt256Bytes, TicketData.Footage] =
    new StoreIndexSwayInterpreter(Paths.get("sway", "transaction", "attachment", "footage"))
  ...
  implicit val licenseTransactionIndex: StoreIndex[IO, (String, UInt256Bytes), Unit] =
    new StoreIndexSwayInterpreter(Paths.get("sway", "transaction", "index", "license"))
  ...
  private val jsonEndpoint = (NodeStatusEndpoint.Get[IO](nodeConfig.networkId, genesisBlock.toHash)
    ...
    :+: TicketEndpoint.Index[IO]
    :+: TicketEndpoint.Post[IO](nodeConfig.networkId, localKeyPair)
  )
