...
trait TransactionRepository[F[_]] {
  ...
  def getAttachment(transactionHash: UInt256Bytes): EitherT[F, String, Option[TicketData.Footage]]
  def put(transaction: Transaction.Verifiable): EitherT[F, String, Unit]
  def putWithAttachment(transaction: Transaction.Verifiable, attachment: TicketData.Footage): EitherT[F, String, Unit]

  def listByAddress(address: Address, offset: Int, limit: Int): EitherT[F, String, List[UInt256Bytes]]
  def listByLicenseNo(licenseNo: String, offset: Int, limit: Int): EitherT[F, String, List[UInt256Bytes]]
}
...
object TransactionRepository {
  ...
  implicit def fromStores[F[_]: Monad](implicit
    ...
    attachmentStore: KeyValueStore[F, UInt256Bytes, TicketData.Footage],
    ...
    def put(transaction: Transaction.Verifiable): EitherT[F,String,Unit] = for {
      ...
      _ <- transaction.value.ticketData.flatMap(_.licenseNo).traverse { licenseNo =>
        EitherT.right[String](licenseTransactionIndex.put((licenseNo, txHash), ()))
      }
    } yield ()

    def putWithAttachment(
      transaction: Transaction.Verifiable,
      attachment: TicketData.Footage
    ...
    def listByLicenseNo(licenseNo: String, offset: Int, limit: Int): EitherT[F, String, List[UInt256Bytes]] =
      licenseTransactionIndex.from((licenseNo, UInt256Refine.EmptyBytes), offset, limit).map(_.map{
